...
def beamSearch(searchAxis, handle, minSumSignal, steps=1000):
    '''
    ...
    if searchAxis == 'x':
        ...
        for x in reversed(Bx[:BxInitIndex]):
        ...
        for y in reversed(By[:ByInitIndex]):
        ...
        for z in reversed(Bz[:BzInitIndex]):
...
def pid(setpoint, position, handle, prePos, integral, initialOutput):
    """
    ...
    xyz.fine_field_cart(xyz.xCoil.coilField, output, handle)

    return (offset, integral, derivative)
...
def kickTracking(handle, eventNumber, allTheData):
